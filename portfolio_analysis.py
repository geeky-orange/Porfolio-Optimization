...
# if os.path.exists(x) and os.path.isdir(x):C:\ProgramData\Anaconda3\envs\pythonProject\python.exe "C:/Users/USER B/Documents/GitHub/portfolio_management/portfolio_analysis.py"
# Traceback (most recent call last):
#   File "C:\Users\USER B\Documents\GitHub\portfolio_management\portfolio_analysis.py", line 12, in <module>
#     import yfinance as yf
...
def get_results_frame(data, number_of_simulations, sortino_target_return):
    ...
    initial_returns  = data.pct_change().dropna() #a copy of the returns to keep it constant during adjustment
    ...
    """
    number_of_columns = 6 #no of columns in the results -- change if more data is needed
    ...
    for i in list(range(num_portfolios)):
        ...
        # confidence levels for the VAR & CVAR -- make a list of confidence levels to have 90,95,99
        confidence = 90
...
def get_category_data(listticker, number_of_years, minimum_target_return, number_of_simulations,
    ...
    if listticker:
        ...
        portfolio_manager(input_data=getdata,
...
#     S = risk_models.sa for maximum Sharpe Ratio
...
def simulate(data, risk_profile, number_of_years, number_of_simulations, finalportfoliotickers):
    ...
    print("printing get category data --------------------------------")
    ans = get_category_data(listticker=get_corporate_bonds(data=data),
                                                       number_of_years=number_of_years, minimum_target_return=0,
                                                       number_of_simulations=number_of_simulations,
                                                       number_of_tickers_returned=5)
...
if __name__ == '__main__':
    ...
    gov_data = get_government_bonds(data)
    usable_data = get_data(gov_data)

    get_results_frame(usable_data, number_of_simulations = 25000, sortino_target_return = 0)
    ...
    # finalmergeddf.to_dict('{}FOR surving i have to write this line'.format(filenamen))
